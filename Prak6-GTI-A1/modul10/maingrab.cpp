...
int posMBadan=0, posYBadan=7, posXKotak=0, posYKotak=6;
float rotTangan1=0.0, rotTangan2=0.0, rotTangan3=0.0, rotTangan4=0.0;
int kick=0, roll=0, hit=0, bring=0, drop=0, biring=0, grab=0,
    tabrak=0;
float jarak=1;
int gerakTangan=0;
...
const GLfloat light_position[] = { 0.0f, 20.0f, 0.0f, 1.0f };
...
void keyboard(unsigned char key, int x, int y)
{
    switch (key)
    {
        ...
        case 'e':
            rotAngleZ -= 2;  break;
        case 'o':
            if(drop==0){      //kondisi jika kotak tidak jatuh saat dipegang
                if (posMBadan > 4){ //kondisi jika tidak menabrak meja
                    posMBadan -= 1;
                    if (bring == 1){
                        posXKotak -= 1;
                    }
                }else{
                    if (posMBadan >posXKotak+3){ //kondisi jika tidak menabrak kotak
                        posMBadan -= 1;
                    }
                }
            }
        ...
        case 'g':
            gerakTangan = 1; break;
        case 't':
            if (posMBadan>=8){ //kondisi kotak telah diluar meja
                drop = 1;
                gerakTangan = 0;
            }
            break;
        case 32:
            ...
            posMBadan=0, posXKotak=0, posYBadan=7, posYKotak=6;
            rotTangan1=rotTangan2=rotTangan3=rotTangan4=kick=roll=gerakTangan=
            drop=hit=biring=grab=0;
            break;
        case 27:
            exit(0);
        default:
...
void pengubahTangan(){
    if (posMBadan==4){      //kondisi jika tubuh menyentuh kotak bawah
        hit = 0;
    }else {
        hit = 1;
    }
    ...
    // memegang
    if(gerakTangan == 1){ //kondisi peleberan tangan samping
        if (rotTangan1 >= -90){
            rotTangan1 -= 0.1;
        }
        if (rotTangan1 < -90){
...
void Object()
{
    ...
    glPushMatrix();
        ...
        glTranslatef(posMBadan,posYBadan,0);
        Balok(3,3,7); // badan
        // tangan kiri
        glPushMatrix();
...
void display() {
    ...
    // Kalau move dan angle tidak nol, gerakan kamera...
    if (deltaMove)
