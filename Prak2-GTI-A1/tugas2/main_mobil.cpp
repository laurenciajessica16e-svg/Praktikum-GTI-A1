...
void road(){
    ...
    float dx = carX - startX;
    float dy = carY - startY;
    ...
    float width = 0.12f;
    float backLength = len;

    glColor3f(0.4f, 0.4f, 0.4f);
    glBegin(GL_QUADS);
        glVertex2f(startX + px*width - dx*backLength, startY + py*width - dy*backLength);
        glVertex2f(startX - px*width - dx*backLength, startY - py*width - dy*backLength);
        glVertex2f(startX - px*width, startY - py*width);
        glVertex2f(startX + px*width, startY + py*width);
    glEnd();
    ...
    glLineWidth(3);
    glBegin(GL_LINES);
    for(float t = 0; t > -backLength; t -= 0.2f){
        glVertex2f(startX + dx*t, startY + dy*t);
        glVertex2f(startX + dx*(t-0.1f), startY + dy*(t-0.1f));
    }
    glEnd();
}
...
void car(){
    ...
    float dx = carX - startX;
    float dy = carY - startY;
    float angle = atan2(dy,dx) * 180.0f / 3.1416f;
